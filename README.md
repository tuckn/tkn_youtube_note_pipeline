...
flowchart LR
    ...
    Source --> Summary["Codex で要約ノートを作成"]
```
...
- 要約を作成する場合は、実行環境から呼び出せる認証済みの Codex CLI。
- 字幕を取得できる YouTube 動画の URL と、取得データ・ノートを書き込める保存先。

以下の操作例は Windows PowerShell 向けです。
他の OS での実動作は未検証です。
字幕取得に使う Python 版 `yt-dlp` は依存パッケージとしてインストールされるため、別途コマンドを導入する必要はありません。
...
コマンド一覧が表示されれば、CLI を呼び出せています。
このインストールでは、その時点のコードを使用します。
...
Codex の事前実行確認も行わないので、認証や実際の生成が成功することまでは確認できません。

### 要約を作成する

**通常実行では YouTube から字幕と動画情報を取得し、要約が必要な場合は動画タイトル・URL・字幕全文を Codex に渡します。**
...
`summary_profile` は要約の言語・構成を選ぶ設定です。
取得する字幕の言語は変わりません。
...
| Codex の起動・生成に失敗する | `codex_executable`、認証、モデル設定を確認します。診断ログがあれば、レポートの `diagnostic_log` を参照します。 |
| 生成がタイムアウトする | `provider_timeout_seconds` と診断ログを確認し、必要なら待機上限を変更して `build-summary` を再実行します。 |
...
| 要約を作成 | `build-summary "<source-note>"` | 生成が必要な場合に Codex を呼び、Markdown とレポートを保存 |
...
| 有効な設定を確認 | `config show` | 読み込んだ設定元、最終値、要約プロファイルの詳細を表示 |
...
生成プロセスの失敗は短い要点を表示し、取得できた詳細出力は別の `*.provider.log` に保存します。
そのログにはプロンプトや字幕が含まれる場合があります。
...
次は `config init` が作成する設定の全体です。
[同梱の設定例](src/youtube_note_pipeline/resources/config.example.yaml)でも確認できます。
...
| `reports_root` | 実行レポート、診断ログ、移行バックアップの保存先 |
| `provider` | 現在の要約生成は `codex` のみ対応 |
| `model` | 次に生成するときのモデル。`null` は Codex が選ぶモデルを使用 |
| `summary_profile` | `default-ja` は日本語、`default-en` は英語の要約 |
| `fallback_languages` | 原語の字幕が使えない場合に試す言語の順序。`[]` は代替言語なし。`[en]` なら英語を許可 |
| `codex_executable` | 起動する Codex CLI。`codex` は実行環境の PATH から探し、絶対パスでも指定可能 |
| `provider_timeout_seconds` | 1回の要約生成プロセスを待つ上限秒数。正の有限値を指定 |

保存先を変えても、既存ファイルは自動移動しません。
別の保存先に同じノートがあっても検索対象外になるため、再生成や重複の原因になります。
モデルの変更だけでは既存要約を自動再生成しません。
変更後のモデルで作り直す場合は、入力と保存先を確認して `build-summary --force` を使います。

保存先、モデル、要約プロファイル、タイムアウトは、それぞれ `--raw-root`、`--source-root`、`--summary-root`、`--reports-root`、`--model`、`--summary-profile`、`--provider-timeout-seconds` でも上書きできます。
利用できるオプションはコマンドごとの `--help` で確認してください。

Windows の自動処理と対話的な PowerShell で異なる `codex` が選ばれる場合は、設定の `codex_executable` に動作する実行ファイルの絶対パスを指定します。
...
sequenceDiagram
    ...
    Config-->>App: 保存先・summary_profile・Codex の設定
    App->>App: 個別の CLI オプションを上書き
    App->>App: 値を検証し、相対保存先を解決
    Note over App: 保存先、使用プロファイル、実行ファイル、モデル、待機上限が決まる
```

ユーザー共通設定は `~/.tkn/youtube_note_pipeline/config.yaml`、作業フォルダの設定は `./.tkn/config.yaml` です。
`summary_profile` が生成指示と出力形式の組を選び、`codex_executable`・`model`・`provider_timeout_seconds` が生成AIの呼び出し条件を決めます。
...
#### 3. プロファイルを読み、Codex 経由で生成AIを呼び出す
...
sequenceDiagram
    ...
    participant Source as source_root
    participant Temp as OS の一時フォルダ
    ...
    App->>Codex: codex_executable で --version を実行
    Codex-->>App: 起動確認の結果
    App->>Temp: 応答形式を summary.schema.json に保存
    App->>Codex: codex exec に指示と入力を標準入力で渡す
    Note over App,Codex: スキーマ・出力先・必要ならモデルを指定し、時間制限付きで待つ
    Codex->>Temp: summary.schema.json を読む
    Codex->>AI: 設定された接続先へ生成を要求
    AI-->>Codex: 生成結果
    Codex->>Temp: summary.json に結果を保存
    Codex-->>App: 終了コード・実行ログ
    App->>Temp: summary.json を読む
    Temp-->>App: 生成された JSON
    App->>App: 応答を出力スキーマで検証
    App->>Temp: この実行で作成した一時フォルダを削除
```

生成AIへの接続と認証は、起動した Codex CLI に任せます。
この図の Codex CLI と生成AIサービス間は、アプリケーションから見た役割を示しており、Codex 内部の個々の通信を表すものではありません。
`model: null` の場合、アプリケーションからモデルを指定せず、Codex 側の選択を使います。
一時フォルダには応答スキーマと生成結果を置き、恒久的なノートの保存には使いません。
...
要約を再利用できる場合は、Codex の起動確認と生成AIの呼び出しを省略します。
途中で失敗した場合は後続処理へ進まず、`ingest` の失敗レポートを `reports_root` に保存します。
生成プロセスの詳細出力を取得できた場合は `*.provider.log` も保存します。
...
| 要約生成 | Codex CLI を使用し、日本語・英語の組み込みプロファイルに対応 |
...
実際の YouTube や Codex での確認は別途行う操作であり、通常のテスト成功は認証・通信・生成品質の確認を意味しません。
...
生成条件の記録、外部プロセスの実行、ファイル保存、生成物の検証もアプリケーションが担当します。
