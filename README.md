...
要約ノートは Frontmatter、タイトル、動画への参照に続き、`1. Summary` → `2. Conclusion` → `3. Key points` → `4. Structuring (from abstract to concrete)` → `5. Technical terms` の順に並びます。概要と結論から読み始め、必要に応じて論点・根拠・用語を確認できます。
別に保存する文字起こしノートでは、取得した字幕全文を時刻付きの段落で読めます。

新規取得時の `manifest.video.thumbnail` と、文字起こしの新規作成・再生成時の `cover` には、
動画 ID から作った通常版の `https://i.ytimg.com/vi/<video-id>/maxresdefault.jpg` を使います。
取得ライブラリが言語別の画像 URL を返しても採用せず、元のメタデータは raw に保持します。
画像のダウンロードや HTTP 存在確認は行いません。
既存ノートは自動で書き換えず、要約の `cover` は入力の文字起こしノートから引き継ぎます。
...
`tkn-youtube-note --version` でインストール済みのバージョンを確認できます。
...
字幕の言語・手動/自動の優先順位は維持し、各候補群では自動翻訳されていない字幕を優先します。
同じ言語に複数の JSON3 がある場合や、`ja-orig` などの別候補がある場合も、URL に翻訳先の `tlang` パラメーターがない字幕を先に選びます。
翻訳字幕しかない場合は従来どおり取得を試みますが、YouTube の自動翻訳字幕への制限によって 429 になることがあります。
...
| 要約のセクションを並べ替え | `reorder-notes` | 再生成せず、結論を前方へ移動。原本をバックアップして保存 |
...
```shell
tkn-youtube-note reorder-notes --dry-run
tkn-youtube-note reorder-notes
```

`summary_root` 配下を再帰的に確認し、従来の5セクションを新しい順序へ移します。生成AIは呼び出しません。変更するのはセクションの位置と見出し番号だけで、各節の文章、リンク、末尾の独立した追記、Frontmatter 全体（生成情報、`reviewStatus`、`noteId`、`date`、`updated` を含む）を保持します。元の改行形式とセクション間の空行も保持します。

`--dry-run` はファイルを作成せず計画を表示します。通常実行は全対象の原本を `reports_root/section-order/<run>/backups/` に保存してから書き込みます。`plan.json` と `result.json` にハッシュ、変更対象、バックアップとの対応を記録します。処理途中でファイルの変更を検出すると停止し、完了済みの対象と原本は同じレポートから確認できます。`reports_root` は Vault 外を指定してください。

既に新しい順序のノートは `unchanged`、5セクションがそろわない旧形式は `skipped` です。重複見出しや途中に別の章が挟まった構成など、曖昧な対象は `blocked` とし、1件でもあれば書き込みません。欠けた節の新規作成や内容の分類し直しは行いません。

日本語・英語プロファイルの新規出力はテンプレート 1.1 を使用します。生成指示と出力JSONスキーマは変更していません。並べ替えた既存ノートは元の生成情報を保持するため、`status` では旧テンプレートとの差が表示されますが、対応する旧契約のノートは新しい順序でも検証できます。生成条件の不一致による `--force` の保護も維持します。章番号を含む見出しへのリンクは自動更新しないため、利用している場合はリンク先を確認してください。
